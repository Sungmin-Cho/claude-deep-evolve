...
- **메타 아카이브** (`~/.deep-evolve/meta-archive/`): 모든 프로젝트에 걸친 검증된 전략의 공유, flock 보호 아카이브입니다.
...
- **재개 (Resume)**: 언제든 중단하세요. `/deep-evolve`를 다시 실행하면 활성 세션을 감지하고, 무결성 검사(브랜치 정렬, dirty tree, orphan 실험)를 수행한 후 이어서 진행합니다.
