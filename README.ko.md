...
- **평가 harness** — 고정된 ground truth. 실험 중 에이전트가 절대 수정하지 않습니다. 두 형태: 평가기 파일 `prepare.cjs`와 검증된 config(CLI 메트릭), 또는 `prepare-protocol.md`(고정 도구 프로토콜).
...
| 평가 | `prepare.cjs` + config | 시나리오, 난이도, 커버리지 | plateau 시 자동 트리거 |
