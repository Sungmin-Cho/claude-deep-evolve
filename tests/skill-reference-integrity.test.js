...
test('a variable-rooted path that resolves in the plugin uses the one anchor spelling', () => {
  ...
  for (const file of markdownFiles()) {
    fs.readFileSync(file, 'utf8').split('\n').forEach((line, i) => {
      ...
      while ((m = VAR_ROOTED.exec(line))) {
        ...
        if (!PLUGIN_FILES.has(normalizeSeparators(m[2]))) continue;
  ...
  assert.ok(rogue && !ANCHOR_VARS.has(rogue[1]) && PLUGIN_FILES.has(normalizeSeparators(rogue[2])),
  ...
  assert.ok(workspace && !PLUGIN_FILES.has(normalizeSeparators(workspace[2])),
