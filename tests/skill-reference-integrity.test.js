...
const ANCHOR_VARS = new Set(['CLAUDE_PLUGIN_ROOT', 'PLUGIN_ROOT']);
...
test('a variable-rooted path that resolves in the plugin uses a sanctioned anchor', () => {
  ...
  // `normalizeSeparators` collapses separators but leaves `.` and `..` alone, so an
  ...
  const resolveTail = (t) => normalizeSeparators(t).split('/').reduce((acc, seg) => {
