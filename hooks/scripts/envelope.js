 ...
 * cf. deep-suite/schemas/handoff.schema.json +
 ...
 *
 * opts:
 *   artifactKind     'evolve-receipt' | 'evolve-insights' (required)
 *   payload          plain object (required, non-null, non-array)
 *   parentRunId      optional ULID — cross-plugin chain (e.g. recurring-findings.run_id)
 ...
 *
 * Defends against legacy receipts whose top-level numeric `schema_version` or
 * absent envelope key collide with envelope's `schema_version: "1.0"`.
 */
 ...
 *
 * Identity is checked on producer === 'deep-evolve', artifact_kind === expectedKind,
...
function unwrapEnvelope(obj, expectedKind) {
  ...
  if (
    ...
    process.stderr.write(
      `[deep-evolve/envelope] identity mismatch: expected producer=${PLUGIN_NAME} kind=${expectedKind}, got ${JSON.stringify(id)}\n`,
    );
    return null;
  }
  ...
  if (payload === null || typeof payload !== 'object' || Array.isArray(payload)) {
    ...
    process.stderr.write(
      `[deep-evolve/envelope] corrupt payload: expected object, got ${
