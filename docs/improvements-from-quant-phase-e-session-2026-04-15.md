...
2. **명시적 자동화 플래그** — `session.yaml.outer_loop.auto_trigger: bool` 추가 (runtime state와 동일 위치)
...
### 3.B `session.yaml.outer_loop.auto_trigger` 플래그 추가 (즉시, 문서 전용, 코드 0줄)

**현재 문제**: 플러그인이 "자동 vs 수동"을 의미론적으로만 정의. Claude가 실행 시 편차 발생.

**제안**: runtime 상태와 동일 위치인 `session.yaml.outer_loop`에 다음 필드 추가 (default: true).

```yaml
# session.yaml (init.md Step 9에서 생성)
outer_loop:
  generation: 0
  interval: 20
...
**위치 선택 이유**: outer loop의 runtime state(`generation`, `q_history`)가 이미 `session.yaml`에 있다. `auto_trigger`를 `strategy.yaml`에 두면 Tier 1이 strategy.yaml을 mutate하는 동안 제어 플래그가 같은 파일에 있어 self-mutating control flow가 되고, session.yaml과 split-brain 관계 형성.
...
**소비 경로**: 이 필드는 Claude가 inner-loop.md의 prose를 해석하며 읽는 용도로만 쓰임. shell/script parsing 없음 → PR#1은 문서 전용 변경 유지.
...
- If the user's initial brief overrides this default with "ask before outer
  loop" or similar language, set session.yaml.outer_loop.auto_trigger=false
