...
command -v flock >/dev/null 2>&1 || FLOCK_AVAILABLE=0  # macOS: Homebrew flock 또는 mkdir-based fallback
```

**Project-level lock (AH1 반영)**: `start_new_session`과 `mark_session_status`는 ID 선점/status 전이가 원자적이어야 한다. project lock을 사용:
...
acquire_project_lock() {
  ...
  while ! mkdir "$lockdir" 2>/dev/null; do
    retries=$((retries - 1))
    [ $retries -le 0 ] && { echo "session-helper: lock timeout" >&2; return 1; }
    sleep 0.5
  done
  trap 'rmdir "$lockdir" 2>/dev/null; cleanup $?' EXIT
}
```

mkdir-based lock은 macOS/Linux 모두 atomic. `flock`이 있으면 `flock` 사용, 없으면 mkdir fallback.

**Status reconciliation (AH2 반영)**: `session.yaml.status`가 routing/hook의 유일한 권위. `sessions.jsonl`은 audit log. `resolve_current`는 startup 시 두 값이 불일치하면 `session.yaml.status`를 trust하고 `sessions.jsonl`에 reconciliation 이벤트 append:

```bash
# resolve_current 내부
expected_status=$(replay sessions.jsonl | jq -r ".\"$SESSION_ID\".status")
actual_status=$(yq .status "$SESSION_ROOT/session.yaml")
if [[ "$expected_status" != "$actual_status" ]]; then
  append_sessions_jsonl "reconciled" "$SESSION_ID" "{\"from\":\"$expected_status\",\"to\":\"$actual_status\"}"
fi
```
...
| `resolve_current` | — | current.json → session_id + session_root 출력. dir 존재 검증 (X6). startup reconciliation 수행. 부재/orphan 시 exit 1 | N/A |
| `list_sessions` | `[--status=X]` | sessions.jsonl 리플레이 → JSON array stdout | N/A |
| `start_new_session` | `<goal> [--parent=<id>]` | **locked**: acquire_project_lock → compute_id → mkdir → append sessions.jsonl → write current.json. 실패 시 namespace dir 삭제 후 lock 해제 | Yes |
| `mark_session_status` | `<id> <status>` | **locked**: append sessions.jsonl status_change + update session.yaml.status. 한 쪽 실패 시 다음 resolve_current에서 reconciliation 보장 | Yes |
| `append_sessions_jsonl` | `<event> <id> <json_fields>` | 한 줄 append (O_APPEND atomic). 의도적 --dry-run 지원 추가 (design doc 원안에 없었으나, state-mutating이므로 spec에서 보강) | Yes |
...
Design doc §3.3.2 + 아래 수정 사항 반영.

**AH3 반영 — resume "proceed on current" = branch rebind (inner-loop 하드 가드와의 충돌 해결)**:

현재 inner-loop.md는 branch alignment을 hard guard로 검사한다 (lines 45-55). resume에서 "proceed on current branch" 를 선택해도 inner-loop 재진입 시 다시 block되므로, "proceed on current"는 단순 경고 dismiss가 아니라 **branch rebind**로 정의한다:

```
Step 3.a에서 "proceed on current branch" 선택 시:
  1. session.yaml.lineage.current_branch를 현재 브랜치로 갱신
  2. journal.jsonl에 branch_mismatch_accepted + branch_rebound 이벤트 기록
  3. receipt.runtime_warnings에도 기록 (X13)
  4. inner-loop.md의 branch guard를 통과할 수 있도록 invariant 복원
  → inner-loop 재진입 시 branch alignment 충족됨
```

이로써 "warn-only" 가 아닌 "explicit rebind with audit trail" — 사용자의 의식적 선택이되 safety invariant은 유지.

**M1 반영 — inner_count Section C reset 조건부화**:

inner-loop.md Section C의 기존 "Set inner_count to 0"은 resume 시에도 reset하면 persist 무의미. 변경:
```
Set inner_count to session.yaml.outer_loop.inner_count
(0 for new sessions, restored value for resume).
```

outer-loop.md의 "Reset inner_count to 0"은 post-outer-loop 이므로 그대로 유지 (generation 전환 후 정상 동작).

#### `skills/deep-evolve-workflow/protocols/history.md`

Design doc §3.4 그대로. MODE=list|detail|lineage|export.

**M5 반영**: detail 모드에서 `--full` 플래그 지원. 기본은 experiments_table top-10 + bottom-5. `--full`로 전수 출력.
...
**Step 2~4**: 기존 `.deep-evolve/` 생성 → `.deep-evolve/<session-id>/` 생성으로 변경. `session-helper.sh start_new_session` 호출. **M3 반영**: mkdir 목록에 `meta-analyses/` 추가 (`runs/`, `code-archive/`, `strategy-archive/`, `meta-analyses/`).
...
**M2 반영**: session.yaml 내 version 필드 (또는 init.md 상단 주석)에 `deep_evolve_version: "2.2.0"` 기록.
...
**경로 치환 (I1 반영)**: completion.md도 `$SESSION_ROOT` 치환 대상. 구체적:
- `.deep-evolve/report.md` → `$SESSION_ROOT/report.md`
- `.deep-evolve/evolve-receipt.json` → `$SESSION_ROOT/evolve-receipt.json`
- `.deep-evolve/results.tsv` → `$SESSION_ROOT/results.tsv`
- `.deep-evolve/session.yaml` → `$SESSION_ROOT/session.yaml`
- `.deep-evolve/journal.jsonl` → `$SESSION_ROOT/journal.jsonl`
- `.deep-evolve/code-archive/` → `$SESSION_ROOT/code-archive/`
- `.deep-evolve/strategy-archive/` → `$SESSION_ROOT/strategy-archive/`
...
**M4 반영 — receipt 쓰기 순서 명시**: write with `outcome: null` → user가 merge/PR/keep/discard 선택 후 `outcome` 업데이트 → freeze (이후 수정 불가). 이 write-update-freeze 시퀀스는 v2.1.x 동작과 동일.
...
**M2 반영**: 프로토콜 라우팅 테이블에 `resume.md`, `history.md` 추가. version bump `1.0.0` → `2.2.0`. 워크플로우 설명에 세션 히스토리 언급.
