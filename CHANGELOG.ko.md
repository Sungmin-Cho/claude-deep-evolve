...
- 워크플로우 지침이 지목하는 모든 에이전트 정책·프로토콜 파일을 명시적인 플러그인 루트 경로로 읽습니다. 실험 대상 프로젝트가 같은 상대 경로에 파일을 두어 해당 지침을 다른 파일로 돌릴 수 없습니다.
- 패키지 런타임 디스패처도 동일한 명시적 플러그인 루트 경로로 실행하여, 같은 방식의 코드 실행 경로를 차단합니다.
- 워크플로우 지침이 플러그인 루트를 지침 문서를 읽은 위치에서 유도하지 않고 호스트 환경에서 가져옵니다. 루트 자체를 워크스페이스가 제공할 수 없습니다.
- 아직 닫히지 않은 부분: seed dispatch context는 여전히 정책 위치를 워크스페이스 상대 경로로 전달하며, 이는 방금 배정된 seed가 아무것도 읽기 전에 가진 유일한 포인터입니다. 이 경로를 통한 대체는 별도 수정 전까지 가능합니다.

## [3.6.1] — 2026-07-27

### Fixed

- 낡은 활성-세션 포인터가 엄격 코덱 이전 레거시 세션을 가리켜 상태 문서를 더 이상 파싱·검증할 수 없을 때 readonly 가드가 영구히 fail-closed로 무관한 요청을 거부하던 문제를 수정합니다. 런타임 자체 세션 생명주기 레지스트리에 해당 세션의 종결 상태(completed 또는 aborted)가 기록되어 있으면 세션을 비활성으로 간주합니다.
- 그보다 약한 증거 — 레지스트리 부재·손상·비종결 상태, 다른 세션의 기록, 포인터 없는 flat 레이아웃 — 는 기존 fail-closed 거부를 유지하며, 읽을 수 있는 active 상태 문서는 항상 레지스트리보다 우선합니다.

## [3.6.0] — 2026-07-19

### Added

- 플러그인 루트 환경변수가 없을 때 가드 훅 시작이 명확한 진단 메시지를 남깁니다.

### Changed

- 양 호스트 표면의 훅 명령이 템플릿 확장 대신 실행 시점 환경변수로 플러그인 루트를 해석하여, 템플릿 변수를 확장하지 않는 호스트에서의 시작 크래시가 제거됩니다.

### Fixed

- 가드 인프라 실패(루트 부재, 스크립트 부재, 자식 크래시·타임아웃)가 조용히 보호를 건너뛰지 않고 fail-closed로 동작합니다.
