...
- 에이전트 정책과 프로토콜 파일을 명시적인 플러그인 루트 경로로 읽습니다. 실험 대상 프로젝트가 자신의 워크트리 안에 같은 상대 경로로 파일을 두어 coordinator·seed 정책을 대체할 수 없습니다.
- 패키지 런타임 디스패처도 동일한 명시적 플러그인 루트 경로로 실행하여, 같은 방식의 코드 실행 경로를 차단합니다.
- 플러그인 루트는 호스트 환경에서 가져오며, 워크스페이스·작업 디렉터리·문서를 읽은 위치에서 유추하지 않습니다.
